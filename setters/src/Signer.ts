...
import { SignerProvider } from '@setheum.js/eth-providers';
import { handleTxResponse } from '@setheum.js/eth-providers/lib';
...
const logger = new Logger('setters/0.0.1');
...
export class Signer extends Abstractsigner implements TypedDataSigner {
  ...
  async claimEvmAccount(evmAddress: string): Promise<void> {
    ...
    const data = 'setheum evm:' + Buffer.from(publicKey).toString('hex');
