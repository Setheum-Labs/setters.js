import SETMABI from '@setheum-labs/predeploy-contracts/build/contracts/Token.json';
...
import {
  ...
  signTransaction
} from '@setheum.js/eth-transactions';
...
it('a series of tests', async () => {
  ...
  const setmContract = new Contract(ADDRESS.SETM, SETMABI.abi, account1Wallet);
  ...
  const oneSetm = 10n ** BigInt(provider.api.registry.chainDecimals[0]);
  ...
  /** transfer setm */
  console.log('transfer setm');
  const queryBalance = () => setmContract.balanceOf(account1.evmAddress);
  const balance1: BigNumber = await queryBalance();
  const amount = 100n * oneSetm;
