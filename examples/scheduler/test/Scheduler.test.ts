...
const SCHEDULE_CALL_ABI = require('@setheum-labs/predeploy-contracts/build/contracts/Schedule.json').abi;
const ERC20_ABI = require('@setheum-labs/predeploy-contracts/build/contracts/ERC20.json').abi;
...
describe('Schedule', () => {
  ...
  it('ScheduleCall works', async () => {
    ...
    const erc20 = new ethers.Contract(ADDRESS.SERP, ERC20_ABI, walletTo as any);
    ...
    await schedule.scheduleCall(ADDRESS.SERP, 0, 300000, 10000, 1, ethers.utils.hexlify(tx.data as string));
  ...
  it('CancelCall works', async () => {
    ...
    let current_block_number = Number(await provider.api.query.system.number());
    await schedule.scheduleCall(ADDRESS.SERP, 0, 300000, 10000, 2, ethers.utils.hexlify(tx.data as string));
  ...
  it('RescheduleCall works', async () => {
    ...
    let current_block_number = Number(await provider.api.query.system.number());
    await schedule.scheduleCall(ADDRESS.SERP, 0, 300000, 10000, 4, ethers.utils.hexlify(tx.data as string));
  ...
  it('works with RecurringPayment', async () => {
    const erc20 = new ethers.Contract(ADDRESS.SETM, ERC20_ABI, walletTo as any);
