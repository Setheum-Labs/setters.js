...
Arbitrager example uses [Scheduler](https://wiki.setheum.network/build/development-guide/smart-contracts/advanced/use-on-chain-scheduler),
a built in service, to periodically asses which token to sell and which to buy. It uses [Oracle](https://wiki.setheum.network/build/development-guide/smart-contracts/advanced/use-oracle-feeds)
...
```
docker run --rm -p 9944:9944 setheum/mandala-node:latest --dev --ws-external --rpc-methods=unsafe --instant-sealing  -levm=trace
```
...
Once you run it, the instances of *SETUSD*, *SERP*, *Uniswap Factory* and *Uniswap Router* smart
...
trading pair (in this example we are using SETUSD and SERP and the address of it should change every
...
```
{
  ...
  liquidityPoolAmountSETUSD: '1000000000000000',
  liquidityPoolAmountSERP: '1000000000000000'
}
```
...
- Address of the SETUSD smart contract
- Address of the SERP smart contract
...
3. It schedules the call of the `trigger()` function with the native **Scheduler** smart contract
    ...
    - To read more about how **Scheduler** works, please consult the [wiki](https://wiki.setheum.network/build/development-guide/smart-contracts/advanced/use-on-chain-scheduler)
...
determine which token is should swap for which. The price of *SETUSD* is set to *1000* and the price
of *SERP* is set to *2000*.

The balances of SETUSD and SERP tokens for the **Arbitrager** smart contract and for the liquidity pool
are printed out:
```
{
  arbitrager: '0xDb19dB041b98d4b9a33c2Bf5C4986E8A0e8CC66F',
  amountSETUSD: '10000000000000',
  amountSERP: '10000000000000',
  lpAmountSETUSD: '1000000000000000',
  lpAmountSERP: '1000000000000000'
}
```

We wait for two block and the **Scheduler** calls the `trigger()` function in the **Arbitrager** and
the latter determines to swap *SERP* for *SETUSD*. This can be seen in the next printout:
```
{
  arbitrager: '0xDb19dB041b98d4b9a33c2Bf5C4986E8A0e8CC66F',
  amountSETUSD: '10996006981039',
  amountSERP: '9000000000000',
  lpAmountSETUSD: '999003993018961',
  lpAmountSERP: '1001000000000000'
}
```
...
The script then changes the prices of the tokens and sets the price of *SETUSD* to *2000* and the
price of *SERP* to *1000*. Since we only waited for one block, the `trigger()` has not been called
yet:
```
{
  arbitrager: '0xDb19dB041b98d4b9a33c2Bf5C4986E8A0e8CC66F',
...
{
  arbitrager: '0xDb19dB041b98d4b9a33c2Bf5C4986E8A0e8CC66F',
  amountSETUSD: '12694582322692',
  amountSERP: '7290000000000',
  lpAmountSETUSD: '997305417677308',
  lpAmountSERP: '1002710000000000'
}
```